<script lang="ts">
    ...
    import { type NextPrevious, ConvertDuration } from "../model/date";
    ...
    let microcycle: model.Microcycle = new model.Microcycle({
    ...
    async function updateMicrocycle() {
        microcycle = await GetMicrocycle(start_date, end_date, usr)
    }
